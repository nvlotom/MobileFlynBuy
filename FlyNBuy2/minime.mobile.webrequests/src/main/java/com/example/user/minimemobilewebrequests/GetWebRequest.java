...
public class GetWebRequest implements OnTaskCompleted {
    ...
    /*constructor*/
    public GetWebRequest(String url, ArrayList<String> parameters, int action){
        this.url=url;
    ...
    private String SendRequest(String header){
        ...
        GetWithAsync get= new GetWithAsync(this.url,this.parameters,header,results_transferer);
        String rep="";
        try {
            rep = get.execute().get().toString();
            Log.v("rep ok", rep);
        }
        catch (Exception ex){
            rep="";
